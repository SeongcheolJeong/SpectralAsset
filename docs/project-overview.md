...
- `261` GLB files currently validate with `0` errors and `0` warnings in [../validation/reports/gltf_validation.json](../validation/reports/gltf_validation.json)
...
- the first thirty-nine `P3` coverage-expansion batches raised `traffic_sign` from `18` to `64` standalone assets, `road_furniture` from `4` to `69`, `road_marking` from `4` to `51`, `road_surface` from `4` to `39`, and `traffic_light` from `5` to `34`
