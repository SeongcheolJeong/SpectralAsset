...
- `50` GLB files currently validate with `0` errors and `0` warnings in [../validation/reports/gltf_validation.json](../validation/reports/gltf_validation.json)
...
- the first two `P3` coverage-expansion batches raised `road_furniture` from `4` to `10` standalone assets and `road_marking` from `4` to `9` standalone assets without changing the current material or scenario counts
