...
- `209` GLB files currently validate with `0` errors and `0` warnings in [../validation/reports/gltf_validation.json](../validation/reports/gltf_validation.json)
...
- the first thirty-one `P3` coverage-expansion batches raised `traffic_sign` from `18` to `50` standalone assets, `road_furniture` from `4` to `53`, `road_marking` from `4` to `45`, `road_surface` from `4` to `33`, and `traffic_light` from `5` to `24`
