...
- `343` GLB files currently validate with `0` errors and `0` warnings in [../validation/reports/gltf_validation.json](../validation/reports/gltf_validation.json)
...
- emissive-profile quality summary currently reports `16` `vendor_derived` and `13` `project_proxy` profiles
...
- the first fifty-one `P3` coverage-expansion batches raised `traffic_sign` from `18` to `82` standalone assets, `road_furniture` from `4` to `94`, `road_marking` from `4` to `63`, `road_surface` from `4` to `51`, and `traffic_light` from `5` to `49`
