...
The current generated baseline now exceeds the original `v1.0` count target through the first six `P3` coverage-expansion batches:

- `18` traffic signs
- `5` traffic lights
- `43` roadway and roadside assets split as `8` road surfaces, `14` road markings, and `21` road-furniture assets

The remaining gaps are now mostly about roadside-tail variants, locale or aging sign variants, and measurement quality rather than headline asset count.
...
- `road_surface` and `road_marking` are healthier now, so the next obvious expansion target shifts to roadside-tail variants and secondary signal-support breadth.
...
1. Return to `road_furniture` for utility poles, cabinet/support-tail variants, and specialty sign/signal attachment hardware.
2. Return to `traffic_light` for beacon-style units and richer controller/support variants after the higher-pressure roadside gaps are covered.
...
4. Return to `road_marking` for yellow centerline families, word legends, and additional raised-marker variants after the current secondary batch settles.
