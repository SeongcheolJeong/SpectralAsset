...
The current generated baseline now exceeds the original `v1.0` count target through the first ten `P3` coverage-expansion batches:

- `24` traffic signs
- `5` traffic lights
- `55` roadway and roadside assets split as `8` road surfaces, `20` road markings, and `27` road-furniture assets
...
- `road_marking` has a stronger roadway-breadth starter set now, so the next obvious expansion target shifts to `road_surface`.
...
1. Return to `road_surface` for rural shoulders, stronger construction-zone distress, and unsealed transition variants now that the current marking breadth batch is in place.
2. Return to `traffic_light` for lane-control or rail-specific heads after higher-pressure roadway breadth is covered.
3. Return to `road_furniture` for deeper utility service detail and specialty attachment hardware after higher-pressure roadway breadth is covered.
4. Return to `traffic_sign` for broader locale families and heavier weathering diversity after the roadway backlog settles.
5. Return to `road_marking` later for bus/bike legends, hatched medians, and more specialized directional variants after the broader roadway backlog settles.
