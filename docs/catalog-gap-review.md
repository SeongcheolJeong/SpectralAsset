...
The current generated baseline now exceeds the original `v1.0` count target through the first thirteen `P3` coverage-expansion batches:

- `24` traffic signs
- `13` traffic lights
- `65` roadway and roadside assets split as `13` road surfaces, `20` road markings, and `32` road-furniture assets
...
- `road_furniture` now has a stronger service-tail starter set too, so the next obvious expansion target shifts from support detail back to broader `traffic_sign` depth.
...
1. Return to `traffic_sign` for broader locale families and heavier weathering diversity now that the next roadside-detail batch is in place.
2. Return to `road_marking` later for bus/bike legends, hatched medians, and more specialized directional variants after the broader roadway backlog settles.
