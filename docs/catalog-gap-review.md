...
The current generated baseline now exceeds the original `v1.0` count target through the first seventeen `P3` coverage-expansion batches:
...
- `road_surface` now has explicit rural, dirt-track, bridge-joint, and lane-drop coverage too, so the next obvious expansion target shifts from roadway breadth back to `traffic_light` transit-priority and specialty-control depth.
...
1. Return to `traffic_light` for transit-priority or more locale-specific specialty arrow/control heads now that the road-surface follow-up batch is in place.
2. Return to `road_furniture` after that for niche service-assembly or roadside utility detail beyond the just-completed control-tail batch.
3. Return to `road_marking` after that for boxed transit legends, colored bike/transit surfacing, or more specialized channelization beyond the current bus/bike and hatched-island batches.
4. Return to `traffic_sign` after that for route shields and larger destination stacks beyond the current service-wayfinding panel batch.
5. Return to `road_surface` later for heavier construction staging, bridge approaches, and curb/barrier transition composites once the signal-depth loop comes back around.
