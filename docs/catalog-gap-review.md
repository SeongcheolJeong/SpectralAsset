...
The current generated baseline now exceeds the original `v1.0` count target through the first nineteen `P3` coverage-expansion batches:

- `38` traffic signs
- `17` traffic lights
- `81` roadway and roadside assets split as `18` road surfaces, `26` road markings, and `37` road-furniture assets
...
- with transit-stop furniture now present, the next obvious expansion target shifts away from roadside support and toward `road_marking` curbside/transit-lane legends and colored surfacing detail.
...
1. Return to `road_marking` for boxed transit legends, colored bike/transit surfacing, curbside bay markings, or more specialized channelization beyond the current bus/bike and hatched-island batches.
2. Return to `traffic_sign` after that for route shields and larger destination stacks beyond the current service-wayfinding panel batch.
3. Return to `road_surface` later for heavier construction staging, bridge approaches, and curb/barrier transition composites once the curbside/transit-lane marking pass lands.
4. Return to `traffic_light` later for pedestrian-hybrid or locale-specific specialty heads after the curbside and route-context loop deepens.
5. Return to `road_furniture` later for bus-bay curb modules, passenger-information hardware, and deeper loading-zone detail once the marking pass establishes that curbside context.
