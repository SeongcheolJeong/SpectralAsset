...
- `56` traffic signs
- `29` traffic lights
- `133` roadway and roadside assets split as `33` road surfaces, `45` road markings, and `55` road-furniture assets
...
1. Return to `road_furniture` next for shelter accessory clusters, curb separators, and more microscale attachment-hardware follow-up now that the cantilever/specialty-signal pass has landed.
2. Return to `road_marking` later for more locale-specific school-zone, curbside, and retroreflective follow-up after the next roadway-support loop lands.
3. Return to `road_surface` later for additional retaining-wall, roundabout-edge, and temporary-staging composites after the next furniture loop lands.
4. Return to `traffic_sign` later for multilingual service follow-up, larger specialty assemblies, and measured retroreflective behavior once the next furniture-support pass lands.
5. Return to `traffic_light` later for narrower regional edge cases, more cantilever subtype variation, and measured emitter follow-up after the roadside-support loop stabilizes.
