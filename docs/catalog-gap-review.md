...
- with pedestrian-hybrid, bicycle, and tram-priority heads now present, the next obvious expansion target shifts away from signal breadth and toward `road_furniture` bus-bay curb modules, passenger-information hardware, and loading-zone detail.
...
1. Return to `road_furniture` for bus-bay curb modules, passenger-information hardware, and deeper loading-zone detail once the latest signal and surface loops establish that curbside context.
2. Return to `road_marking` later for turn-pocket stencils, conflict-zone surfacing, and curb-color/loading delimiters after the next curbside-detail pass lands.
3. Return to `traffic_sign` later for overhead destination assemblies or additional locale-specific route-shield families after the roadway-context loop deepens.
4. Return to `road_surface` later for grade-separated ramp ties, median noses, and deeper temporary work-zone composites after the next signal and curbside loops land.
5. Return to `traffic_light` later for controller nuance, rail-gate assembly context, or locale-specific edge-case heads after the curbside-detail loop lands.
