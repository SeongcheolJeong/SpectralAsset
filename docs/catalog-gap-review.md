...
- `50` traffic signs
- `24` traffic lights
- `119` roadway and roadside assets split as `28` road surfaces, `38` road markings, and `53` road-furniture assets
...
- with the curbside/attachment furniture pass now landed, the next obvious expansion target shifts from `road_furniture` back toward `road_marking` turn-pocket stencils, school-bus queue markings, and curbside-arrow follow-up.
...
1. Return to `road_marking` next for turn-pocket stencils, school-bus queue markings, curbside arrows, and other follow-up lane-control detail now that the curbside/furniture pass has landed.
2. Return to `road_surface` later for roundabout aprons, retaining-wall tie-ins, and deeper temporary work-zone composites after the next marking and roadway loops land.
3. Return to `traffic_sign` later for more localized service panels, specialty assemblies, and measured retroreflective depth once the roadway-context loop deepens again.
4. Return to `traffic_light` later for niche regional heads, cantilevered crossing detail, and measured emitter follow-up once the broader roadside-context loop lands.
5. Return to `road_furniture` later for shelter accessory clusters, curb separators, and more microscale attachment-hardware follow-up once the next curbside-marking pass lands.
