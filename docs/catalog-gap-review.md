...
The current generated baseline now exceeds the original `v1.0` count target through the first fourteen `P3` coverage-expansion batches:

- `31` traffic signs
- `13` traffic lights
- `65` roadway and roadside assets split as `14` road surfaces, `20` road markings, and `31` road-furniture assets
...
- `traffic_sign` now has broader locale and weathering depth too, so the next obvious expansion target shifts from sign breadth back to more specialized `road_marking` work.
...
1. Return to `road_marking` for bus/bike legends, hatched medians, and more specialized directional variants now that the broader sign-depth batch is in place.
2. Return to `traffic_sign` later for route/service families and more locale-specific wayfinding panels after the roadway-specialization backlog settles.
3. Return to `road_surface` after that for crowned rural lane panels, dirt-track variants, and bridge/joint specialty panels rather than the just-completed shoulder-and-distress batch.
4. Return to `traffic_light` later for transit-priority or more locale-specific specialty heads after the support-detail backlog settles.
5. Return to `road_furniture` after that for niche service-assembly or roadside utility detail rather than the just-completed control-tail batch.
