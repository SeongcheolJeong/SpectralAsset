...
The current generated baseline now exceeds the original `v1.0` count target through the first twenty-one `P3` coverage-expansion batches:
...
- with route shields and larger destination-guide stacks now present, the next obvious expansion target shifts away from sign breadth and back toward `road_surface` heavier construction staging, bridge approaches, and curb/barrier transition composites.
...
1. Return to `road_surface` for heavier construction staging, bridge approaches, and curb/barrier transition composites now that curbside/transit-lane markings and larger guide signs are in place.
2. Return to `traffic_light` later for pedestrian-hybrid or locale-specific specialty heads after the roadway-context loop deepens.
3. Return to `road_furniture` later for bus-bay curb modules, passenger-information hardware, and deeper loading-zone detail once the surface and curbside loops establish that context.
4. Return to `road_marking` later for turn-pocket stencils, conflict-zone surfacing, and curb-color/loading delimiters after the next road-surface pass lands.
5. Return to `traffic_sign` later for overhead destination assemblies or additional locale-specific route-shield families after the roadway-context loop deepens.
