...
The current generated baseline now exceeds the original `v1.0` count target through the first twenty-nine `P3` coverage-expansion batches:

- `50` traffic signs
- `21` traffic lights
- `104` roadway and roadside assets split as `23` road surfaces, `38` road markings, and `43` road-furniture assets
...
- with the overhead destination-assembly and locale-specific shield follow-up now landed, the next obvious expansion target shifts from `traffic_sign` back toward `road_surface` ramp-tie, median-nose, and temporary work-zone composite follow-up.
...
1. Return to `road_surface` next for grade-separated ramp ties, median noses, and deeper temporary work-zone composites after the latest sign follow-up lands.
2. Return to `traffic_light` later for controller nuance, rail-gate assembly context, or locale-specific edge-case heads after the next roadway-context loop lands.
3. Return to `road_furniture` later for queue rails, shelter ad panels, curb-ramp modules, and more attachment-hardware depth once the next roadway and signal loops are in place.
4. Return to `road_marking` later for turn-pocket stencils, school-bus queue markings, curbside arrows, and other follow-up lane-control detail once the sign and roadway follow-up loops land.
