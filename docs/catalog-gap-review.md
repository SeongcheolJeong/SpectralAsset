...
The current generated baseline now exceeds the original `v1.0` count target through the first nine `P3` coverage-expansion batches:
...
- `traffic_sign` has a stronger depth starter set now, so the next obvious expansion target shifts back to roadway breadth, with `road_marking` first.
...
1. Return to `road_marking` for yellow centerline families, lane-word legends, and additional raised-marker variants now that the sign-depth starter set is in place.
2. Return to `road_surface` for rural shoulders, stronger construction-zone distress, and unsealed transition variants after broader roadway context expands again.
3. Return to `traffic_light` for lane-control or rail-specific heads after higher-pressure roadway breadth is covered.
4. Return to `road_furniture` for deeper utility service detail and specialty attachment hardware after higher-pressure roadway breadth is covered.
5. Return to `traffic_sign` for broader locale families and heavier weathering diversity after the roadway backlog settles.
