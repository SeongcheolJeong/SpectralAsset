...
The current generated baseline now exceeds the original `v1.0` count target through the first thirty-five `P3` coverage-expansion batches:

- `56` traffic signs
- `29` traffic lights
- `145` roadway and roadside assets split as `33` road surfaces, `51` road markings, and `61` road-furniture assets
...
- with the locale-specific school-zone, separator, and curbside marking pass now landed, the next obvious expansion target shifts from `road_marking` toward `road_surface` retaining-wall, roundabout-edge, and temporary-staging follow-up.
...
1. Return to `road_surface` next for additional retaining-wall, roundabout-edge, and temporary-staging composites after the latest marking pass has landed.
2. Return to `traffic_sign` later for multilingual service follow-up, larger specialty assemblies, and measured retroreflective behavior once the next roadway-support pass lands.
3. Return to `traffic_light` later for narrower regional edge cases, more cantilever subtype variation, and measured emitter follow-up after the next roadway-support loop stabilizes.
4. Return to `road_furniture` later for deeper shelter accessory families, broader separator systems, and finer microscale anchors after the next road-surface pass lands.
5. Return to `road_marking` later for even narrower localized school-zone, separator, and curbside stencils after the next road-surface loop lands.
