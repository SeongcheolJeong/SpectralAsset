...
The current generated baseline now exceeds the original `v1.0` count target through the first thirty-eight `P3` coverage-expansion batches:

- `64` traffic signs
- `34` traffic lights
- `153` roadway and roadside assets split as `39` road surfaces, `51` road markings, and `63` road-furniture assets
...
1. Return to `road_furniture` next for cantilever footing detail, rail-side ancillary support, and finer transit-stop microsupport now that the latest signal loop has landed.
2. Return to `road_marking` later for even narrower localized school-zone, separator, and curbside stencils after the next furniture loop lands.
3. Return to `road_surface` later for narrower locale-specific roadway composites and measured behavior follow-up after the next furniture and marking loops land.
4. Return to `traffic_sign` later for gantry-scale guide follow-up and still narrower multilingual/local service breadth after the next furniture loop lands.
5. Return to `traffic_light` later for even narrower regional control-face edge cases after the next furniture loop lands.
