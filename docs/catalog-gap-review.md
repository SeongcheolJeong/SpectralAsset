...
The current generated baseline now exceeds the original `v1.0` count target through the first thirty-one `P3` coverage-expansion batches:

- `50` traffic signs
- `24` traffic lights
- `113` roadway and roadside assets split as `28` road surfaces, `38` road markings, and `47` road-furniture assets
...
| `road_marking` | `38` | part of the expanded post-`v1.0` roadway catalog | expanded | directional arrows, edge lines, worn crossing and stop variants, yellow centerline families, bus/bike legends, hatched median/island panels, white/yellow/bi-color raised-marker strips, boxed curbside/transit legends, red/green colored surfacing panels, curb-color segments, loading zigzag delimiters, and red/green conflict-zone panels now exist alongside the baseline lane and stop geometry | turn-pocket stencils, school-bus queue markings, curbside arrows, and measured retroreflective response | `P3` later after the next furniture pass, `P5` for measured optics |
| `road_furniture` | `47` | part of the expanded post-`v1.0` roadway catalog | expanded | roadside, signal-support, sign-assembly, transit-stop, curbside-loading, and rail-gate context now includes guardrails, bollards, delineators, cones, barriers, barricades, utility poles, signal backplates, side-mount and overhead brackets, cabinet variants, a junction box, service disconnects, meter pedestals, battery-backup cabinets, pull boxes, pad-mount transformers, sign backs, sign mounting brackets, a bus-stop shelter, a stop totem, a bench, a bus-bay curb module, a bus-bay island nose, passenger-information kiosks, real-time arrival displays, loading-zone signposts, loading kiosks, a handhole cluster, protective service bollards, and rail-gate mast/arm/bell/controller detail | queue rails, shelter ad panels, curb-ramp modules, and more attachment-hardware depth | `P3` next for curbside/attachment follow-up |
...
- `road_furniture` now covers much more of the roadside-support tail because the latest batches added utility poles, smaller cabinet/junction variants, service disconnect and metering detail, pull boxes, pad-mount transformers, explicit sign/signal attachment hardware, transit-stop shelter/totem/bench detail, loading-zone curbside context, and now rail-gate mast, arm, bell, and controller-cabinet context.
- with the rail-gate/controller-nuance signal pass now landed, the next obvious expansion target shifts from `traffic_light` toward `road_furniture` queue-rail, shelter-panel, curb-ramp, and deeper attachment-hardware follow-up.
...
1. Return to `road_furniture` next for queue rails, shelter ad panels, curb-ramp modules, and more attachment-hardware depth now that the latest signal loop is in place.
2. Return to `road_marking` later for turn-pocket stencils, school-bus queue markings, curbside arrows, and other follow-up lane-control detail once the next furniture and roadway loops land.
3. Return to `road_surface` later for roundabout aprons, retaining-wall tie-ins, and deeper temporary work-zone composites after the next curbside/furniture pass lands.
4. Return to `traffic_sign` later for more localized service panels, specialty assemblies, and measured retroreflective depth once the roadway-context loop deepens again.
5. Return to `traffic_light` later for niche regional heads, cantilevered crossing detail, and measured emitter follow-up once the broader roadside-context loop lands.
