...
- with the curbside transit/loading detail loop and the curb-color/conflict-zone marking pass now landed, the next obvious expansion target shifts from `road_marking` toward `traffic_sign` overhead destination assemblies and locale-specific shield follow-up.
...
1. Return to `traffic_sign` next for overhead destination assemblies or additional locale-specific route-shield families after the roadway-context loop deepens.
2. Return to `road_surface` later for grade-separated ramp ties, median noses, and deeper temporary work-zone composites after the next sign and curbside loops land.
3. Return to `traffic_light` later for controller nuance, rail-gate assembly context, or locale-specific edge-case heads after the next sign follow-up loop lands.
4. Return to `road_furniture` later for queue rails, shelter ad panels, curb-ramp modules, and more attachment-hardware depth once the current curbside lane-marking and sign-depth loops are in place.
5. Return to `road_marking` later for turn-pocket stencils, school-bus queue markings, curbside arrows, and other follow-up lane-control detail once the sign-expansion loop lands.
