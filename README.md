...
- the fiftieth `P3` expansion batch now adds bus-only lane-control, bar-style transit-priority, diagonal dual-lunar preemption, compact vertical amber-warning, and compact bicycle lane-control heads plus cantilever arm-junction-box and end-cap detail so traffic-light depth no longer stops at the earlier compact multimodal-control and backspan/brace-only cantilever follow-up batch
- the fifty-first `P3` expansion batch now adds slim/aux cantilever controller boxes, pole-tail riser/service-loop hardware, and bus-stop notice/perch accessories so roadside-support depth no longer stops at arm-junction-box/end-cap detail and help-point/request-pole-only transit microsupport
- the fifty-second `P3` expansion batch now adds bicycle-only lane-control, dual-horizontal pedestrian-wait, transit-priority T-face, and bus/tram bar-style signal heads plus cantilever service-conduit and splice-box detail so the signal catalog no longer stops at bus-only lane-control, single wait-indicators, and end-cap-only cantilever service follow-up
- the fifty-third `P3` expansion batch now adds shelter-power pedestals and inverter boxes, ticket-machine and platform-handrail stop utilities, plus signal-base handhole and conduit-riser detail so roadside-support depth no longer stops at notice/perch transit accessories, queue-support context, and pole-tail-only signal-base hardware
- the fifty-fourth `P3` expansion batch now adds `TRAM` and `TRAM STOP` platform legends, `DELIVERY` and `NO STOP` curbside-control boxes, plus left/right `KEEP` separator panels so road-marking depth no longer stops at bus/bike legends, earlier curbside reservations, and arrow/chevron-only separator control
- the fifty-fifth `P3` expansion batch now adds transit-platform bulbouts and median islands, curbside loading/enforcement aprons, plus separator-island boarding-refuge and bus-bay taper surfaces so roadway depth no longer stops at floating-bus-stop, transfer-platform, and earlier separator-island follow-up composites
- the fifty-sixth `P3` expansion batch now adds tram/bus-platform wayfinding panels, taxi/loading-zone service panels, platform/refuge destination stacks, and an overhead platform/refuge guide assembly so sign depth no longer stops at the earlier transfer-wayfinding, multilingual-service, and overhead-guide follow-up sets
- the fifty-seventh `P3` expansion batch now adds bus/tram-platform compact heads, a platform pedestrian-bicycle compact head, taxi/loading-only curbside lane-control faces, and left/right separator-arrow control heads plus dedicated taxi/loading lane-control emissive profiles so traffic-light depth no longer stops at the earlier multimodal bar-style, bus-only, and bicycle-only control follow-up batch
